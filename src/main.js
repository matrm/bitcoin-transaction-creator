...
class StringCache {
	constructor(limit=50, valueLengthLimit=10000) {
		assert(limit > 0);
		assert(valueLengthLimit > 0);
		this.limit = limit;
	...
	add(key, value) {
		...
		assert(!(key in this._strings));
		this._strings[key] = value;
		this._keys.push(key);
		assert(this.limit > 0);
		while (this._keys.length > this.limit) {
			delete this._strings[this._keys.shift()];
		}
	}

	get(key) {
		// If this._keys is ever changed into a linked list it can be moved to the end here to prevent getting removed soon.
		return this._strings[key];
	}
}

const DEFAULT_TIME_BETWEEN_RATE_LIMITED_FETCH_ms = 500;

const key_hostname_value_timeBetween_ms = {};
const key_hostname_value_nextPossibleFetchTime_ms = {};
...
function addHostnameTimeBetween(hostname, timeBetween_ms) {
	assert(hostname);
	assert(timeBetween_ms >= 0);
	key_hostname_value_timeBetween_ms[hostname] = timeBetween_ms;
	key_hostname_value_nextPossibleFetchTime_ms[hostname] = key_hostname_value_nextPossibleFetchTime_ms[hostname] || Date.now();
}

async function fetchRateLimited(url, ...args) {
	const hostname = getHostnameFromURL(url);
	if (!(hostname in key_hostname_value_nextPossibleFetchTime_ms)) {
		addHostnameTimeBetween(hostname, DEFAULT_TIME_BETWEEN_RATE_LIMITED_FETCH_ms);
		console.log(`Automatically added default rate limit of ${DEFAULT_TIME_BETWEEN_RATE_LIMITED_FETCH_ms}ms for fetch()ing from host name "${hostname}".`);
	}
	const timeAvailableValue = key_hostname_value_nextPossibleFetchTime_ms[hostname];
	assert(Number.isFinite(timeAvailableValue));
	const now = Date.now();
	const timeAvailable = Math.max(timeAvailableValue, now);
	const timeDifference = timeAvailable - now;
	assert(timeDifference >= 0);
	assert(hostname in key_hostname_value_timeBetween_ms);
	const timeToAdd = key_hostname_value_timeBetween_ms[hostname];
	assert(Number.isFinite(timeToAdd));
	assert(timeAvailable >= now);
	key_hostname_value_nextPossibleFetchTime_ms[hostname] = timeAvailable + timeToAdd;
	if (timeDifference > 0) {
		console.log(`Sleeping ${timeDifference}ms to avoid getting rate limited from ${hostname}.`);
		await sleep_ms(timeDifference);
	}
	const response = await fetch(url, ...args);
	return response;
}

addHostnameTimeBetween('api.whatsonchain.com', 500);
...
function checkPlainUTXO(utxo) {
	...
	if (!Number.isSafeInteger(utxo.outputIndex) || utxo.outputIndex < 0) {
		throw new Error('UTXO must have a outputIndex property of type number (integer) that is not negative or too big');
	}
...
async function fetchAddressesUTXOsMapWithoutScript(addresses) {
	...
	assert(addresses.length == addressesSet.size);// Check for duplicate addresses.
...
function getSignedTxOutputsAsUTXOs(tx) {
	...
	const utxos = tx.outputs
		.map((output, outputIndex) => {
			assert(output.script instanceof bsv.Script);
			if (!output.script.isPublicKeyHashOut()) {
				return null;
			}
			assert(Number.isSafeInteger(output.satoshis));
			if (!output.satoshis) {
				return null;
			}
			return {
		...
		// Non UTXOs are converted to null earlier so they can be filtered out here.
		// They can't be filtered earlier because outputIndex will be lost.
